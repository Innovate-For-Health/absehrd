...
def _add_subparser_train(subparsers):
...
def _add_subparser_realism(subparsers):
...
def _add_subparser_privacy(subparsers):
...
def create_parser():
    ...
    subparsers = parser.add_subparsers(help='task to perform',
                                    dest='task',
                                    required=True)
    _add_subparser_train(subparsers)
    _add_subparser_generate(subparsers)
    _add_subparser_realism(subparsers)
    _add_subparser_privacy(subparsers)

    return parser

def check_user_input(parser):
...
def check_user_input_train(parser):
...
def check_user_input_generate(parser):
...
def absehrd(parser):
    ...
    status = check_user_input(parser)
    args = parser.parse_args()

    if args.task == 'train':
        check_user_input_train(parser)
    if args.task == 'generate':
        check_user_input_generate(parser)

    if args.task == 'train':
        absehrd_train(parser)
    elif args.task == 'generate':
        absehrd_generate(parser)
    elif args.task == 'realism':
        absehrd_realism(parser)
    elif args.task == 'privacy':
        absehrd_privacy(parser)
