...
parser.add_argument("--n_cpu", type=int, default=1
    , help="number of cpu threads to use during batch generation (default = 1, max = " + str(multiprocessing.cpu_count()) + ')')
...
parser.add_argument("--cuda", type=int, default=0, help="CUDA activation (0/1)")
#parser.add_argument("--multiplegpu", type=int, default=1,
#                    help="use multiple GPU threads to use during batch generation (0/1)")
parser.add_argument("--n_gpu", type=int, default=0, help="Number of GPUs in case of multiple GPU")

parser.add_argument("--latent_dim", type=int, default=128, help="dimensionality of the latent space")
#parser.add_argument("--img_size", type=int, default=28, help="size of each image dimension")
#parser.add_argument("--channels", type=int, default=1, help="number of image channels")
parser.add_argument("--sample_interval", type=int, default=100, help="interval betwen image samples")
parser.add_argument("--epoch_time_show", type=int, default=1, help="interval betwen image samples")
parser.add_argument("--epoch_save_model_freq", type=int, default=100, help="number of epops per model save")
parser.add_argument("--minibatch_averaging", type=int, default=0, help="Minibatch averaging (0/1)")

parser.add_argument("--training", type=int, default=1, help="Training status (0/1)")
parser.add_argument("--resume", type=int, default=0, help="Training status (0/1)")
parser.add_argument("--finetuning", type=int, default=0, help="Training status (0/1)")
parser.add_argument("--n_gen", type=int, default=10000, help="Number of synthetic samples to generate")
parser.add_argument("--generate", type=int, default=1, help="Generating Sythetic Data (0/1)")
parser.add_argument("--evaluate", type=int, default=0, help="Evaluation status (0/1)")
...
parser.add_argument("--outprefix", type=str, default="corgan", help="Prefix for output files")
...
if opt.cuda:
    if torch.cuda.device_count() > 1 and opt.n_gpu > 1:
        n_gpu = min(torch.cuda.device_count(),opt.n_gpu)
        print("Let's use", n_gpu, "GPUs!")
        generatorModel = nn.DataParallel(generatorModel, list(range(n_gpu)))
        discriminatorModel = nn.DataParallel(discriminatorModel, list(range(n_gpu)))
        autoencoderModel = nn.DataParallel(autoencoderModel, list(range(n_gpu)))
        autoencoderDecoder = nn.DataParallel(autoencoderDecoder, list(range(n_gpu)))
    """
    ...
    if multiprocessing.cpu_count() > 1 and opt.n_cpu > 1:
        n_cpu = min(multiprocessing.cpu_count(), opt.n_cpu)
...
if opt.training:
    ...
    if opt.resume:
        ...
        checkpoint = torch.load(os.path.join(opt.expPATH, opt.outprefix + ".model_epoch_" + str(opt.n_epochs) + ".pth"))
    ...
    for epoch in range(opt.n_epochs):
        ...
        with torch.no_grad():
            ...
            accuracy_real_test = discriminator_accuracy(torch.sigmoid(out_real_test), valid)

            out_fake_test = discriminatorModel(fake_samples_test.detach()).view(-1)
            accuracy_fake_test = discriminator_accuracy(torch.sigmoid(out_fake_test), fake)
        ...
        if (epoch + 1) % opt.epoch_save_model_freq == 0 or (epoch + 1) == opt.n_epochs:
...
if opt.generate:
    ...
    gen_samples = np.zeros((opt.n_gen,featureSize), dtype=type(real_samples))
    n_batches = int(opt.n_gen / opt.batch_size) + 1
    #print("n_gen = ", opt.n_gen)
    #print("n_batches = ", n_batches)
    for i in range(n_batches):
        # Sample noise as generator input
        # z = Variable(Tensor(np.random.normal(0, 1, (opt.batch_size, opt.latent_dim))))
        batch_size = min(opt.batch_size, opt.n_gen - i * opt.batch_size)
        #print("i = ",i)
        ...
        gen_samples[i * opt.batch_size:i * opt.batch_size + batch_size, :] = gen_samples_decoded.cpu().data.numpy()
    ...
    # save synthetic data
    np.save(os.path.join(opt.expPATH, opt.outprefix + ".synth.npy"), gen_samples, allow_pickle=False)

if opt.evaluate:
    # Load synthetic data
    gen_samples = np.load(os.path.join(opt.expPATH, opt.outprefix + ".synth.npy"), allow_pickle=False)
